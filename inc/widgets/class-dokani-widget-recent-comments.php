<?php
/**
 * Widget API: Dokani_Widget_Recent_Comments class
 *
 * @package WordPress
 * @subpackage Widgets
 * @since 4.4.0
 ...
 * @see WP_Widget
 */
class Dokani_Widget_Recent_Comments extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'classname'                   => 'dokani_widget_recent_comments',
			'description'                 => __( 'Your site&#8217;s most recent comments.' ),
			'customize_selective_refresh' => true,
			'show_instance_in_rest'       => true,
		);
		parent::__construct( 'dokani-recent-comments', __( 'Dokani: Recent Comments' ), $widget_ops );
		$this->alt_option_name = 'dokani_widget_recent_comments';
    ...
    $limit_post_title = ( ! empty( $instance['limit_post_title'] ) ) ? absint( $instance['limit_post_title'] ) : 5;
        ...
        $post_thumb 	= get_the_post_thumbnail( $comment->comment_post_ID, 'recent-post-widget' );
				$output .= '<li class="recentcomments image-thumb">';
        $output .= '<div class="image">'. $post_thumb .'</div><div class="desc"><span class="comment-author-link">' . get_comment_author_link( $comment ) . '</span>'. esc_attr( ' on ', 'dokani' ) .'<a href="' . esc_url( get_comment_link( $comment ) ) . '">' . wp_trim_words(get_the_title( $comment->comment_post_ID ), $limit_post_title ) . '</a></div>';
				$output .= '</li>';
...
register_widget('Dokani_Widget_Recent_Comments');