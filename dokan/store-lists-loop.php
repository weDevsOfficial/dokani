<div id="dokan-seller-listing-wrap" class="grid-view">
	<div class="seller-listing-content">
		<?php if ( $sellers['users'] ) : ?>
			<ul class="dokan-seller-wrap coloum-<?php echo esc_attr( $per_row ); ?>">
				<?php
				foreach ( $sellers['users'] as $seller ) {
					...
					$store_banner_url  		  = ( $store_banner_id ) ? wp_get_attachment_image_src( $store_banner_id, $image_size ) : get_template_directory_uri() . '/assets/images/single-default-store-banner.png';
							...
							<div class="store-header">
								...
								<div class="featured-favourite">
									<?php if ( $is_store_featured ) : ?>
										<div class="featured-label"><?php esc_html_e( 'Featured', 'dokani' ); ?></div>
									<?php endif ?>
								...
								<?php if ( 'on' === $show_store_open_close && 'yes' === $dokan_store_time_enabled ) : ?>
									<?php if ( dokan_is_store_open( $seller->ID ) ) { ?>
										<span class="dokan-store-is-open-close-status dokan-store-is-open-status" title="<?php esc_attr_e( 'Store is Open', 'dokani' );?>"><?php esc_html_e( 'Open', 'dokani' ); ?></span>
									<?php } else { ?>
										<span class="dokan-store-is-open-close-status dokan-store-is-closed-status" title="<?php esc_attr_e( 'Store is Closed', 'dokani' );?>"><?php esc_html_e( 'Closed', 'dokani' ); ?></span>
									<?php } ?>
								<?php endif ?>
								...
								<div class="store-data-container">
									...
									<div class="store-data <?php echo esc_attr( $store_open_is_on ); ?>">
										...
										<?php if ( !empty( $store_rating['count'] ) ): ?>
											<div class="dokan-seller-rating" title="<?php echo sprintf( esc_attr__( 'Rated %s out of 5', 'dokani' ), esc_attr( $store_rating['rating'] ) ) ?>">
												<?php echo wp_kses_post( dokan_generate_ratings( $store_rating['rating'], 5 ) ); ?>
												<p class="rating">
													<?php echo esc_html( sprintf( __( '%s out of 5', 'dokani' ), $store_rating['rating'] ) ); ?>
												</p>
											</div>
										<?php endif ?>
