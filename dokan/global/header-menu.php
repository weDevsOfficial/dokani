...
<ul class="nav navbar-nav navbar-right">
	...
	if ( is_user_logged_in() ) :
		...
		if ( dokan_is_user_seller( $user_id ) ) :
			?>
			<li class="dropdown">
				<a href="#" class="dropdown-toggle dokani-menu-vendor-dashboard" data-toggle="dropdown"><?php esc_html_e( 'Vendor Dashboard', 'dokani' ); ?> <i class="fas fa-angle-down"></i></a>

				<ul class="dropdown-menu">
					<li><a href="<?php echo esc_url( dokan_get_store_url( $user_id ) ); ?>" target="_blank"><?php esc_html_e( 'Visit your store', 'dokani' ); ?> <i class="fas fa-external-link"></i></a></li>
					<li class="divider"></li>
					<?php
					foreach ( $nav_urls as $key => $item ) {
						printf( '<li><a href="%s">%s &nbsp;%s</a></li>', esc_url( $item['url'] ), wp_kses_post( $item['icon'] ), esc_html( wp_strip_all_tags( $item['title'] ) ) );
					}
					?>
				</ul>
			</li>
		...
		<li class="dropdown">
			<a href="#" class="dropdown-toggle dokani-menu-user" data-toggle="dropdown"><?php echo esc_html( $current_user->display_name ); ?> <i class="fas fa-angle-down"></i></a>
			<ul class="dropdown-menu">
